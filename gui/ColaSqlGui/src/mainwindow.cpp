    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(ui_log, SIGNAL(login()), this, SLOT(refresh()));
...
void MainWindow::refresh()
{
    ...
    int ret = DataProcessor::GetInstance().GetCurrentUser(user_name);
    if (ret != kSuccess)
    {
        qDebug() << "invalid user!";
        assert(false);
    }
    ui->current_user->setText(QString::fromStdString(user_name));
