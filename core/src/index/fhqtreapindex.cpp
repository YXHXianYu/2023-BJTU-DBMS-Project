    ...
    const std::vector<std::string>& compare_key)
{
    ...
    _records_ptr = nullptr;
    _records_ptr =
        new std::vector<std::unordered_map<std::string, std::any>>(records);

    // delete _field_map_ptr;
    _field_map_ptr = nullptr;
    _field_map_ptr =
        new std::unordered_map<std::string, std::string>(field_map);
    ...
    std::vector<int>& result_indexes)
{
    ...
    for (int i = 0, cnt = 0; i < _compare_key.size() && cnt < conditions.size();
         i++)
    {
        int flag = false;
        for (const auto& [name, value, type] : conditions)
        {
            ...
            recordBasedOnConditions.insert(
                {name, ColasqlTool::GetAnyByTypeAndValue(
                           _field_map_ptr->at(name), value)});
            flag = true;
            break;
        }
        if (flag)
        { // this key is in condition
            cnt += 1;
        }
        else
        { // this key is not in condition
            return kFailedIndexCouldnotSpeedup;
        }
    }

    // for(auto& [str, any]: recordBasedOnConditions) {
    //     std::cout << str << ", " << ColasqlTool::AnyToString(any) <<
    //     std::endl;
    // }
...
void FHQTreapIndex::pushup(int x) { t[x].siz = t[LC].siz + t[RC].siz + 1; }
...
void FHQTreapIndex::splitv(int x,
                           const std::unordered_map<std::string, std::any>& va,
                           int& l, int& r, bool nullIsLess)
{
    if (x == 0)
    {
        l = r = 0;
...
void FHQTreapIndex::splitvR(int x,
                            const std::unordered_map<std::string, std::any>& va,
                            int& l, int& r, bool nullIsLess)
{
    if (x == 0)
