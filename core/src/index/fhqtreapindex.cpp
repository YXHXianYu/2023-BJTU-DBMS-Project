...
FHQTreapIndex::FHQTreapIndex(const std::vector<std::unordered_map<std::string, std::any>>& records, const std::vector<std::pair<std::string, std::string>>& field_map, const std::vector<std::string>& compare_key): Index(records, field_map, compare_key) {
    build(records, field_map, compare_key);
}

int FHQTreapIndex::build(const std::vector<std::unordered_map<std::string, std::any>>& records, const std::vector<std::pair<std::string, std::string>>& field_map, const std::vector<std::string>& compare_key) {
...
struct fhqtreap {
	...
	int new_node(int va) {
		tot++;t[tot].c[0]=t[tot].c[1]=0;t[tot].siz=1;
		t[tot].pos=rand()<<15|rand();t[tot].va=va;return tot;
	}
