...
int FileManager::WriteDatabasesFile(const std::vector<Database>& databases)
{
    ...
    if (out.is_open())
    {
        ...
        {
            out << database.GetDatabaseName() << " "
                << database.GetOwnerUserName() << std::endl;
        }
        out.close();
    }
                                 ...
                                 const std::vector<Table>& tables)
{
    ...
    for (const auto& table : tables)
    {
        ...
        out << ColasqlTool::ConstraintsToString(table.GetConstraints())
            << std::endl;
        out.close();
...
int FileManager::WriteUsersFile(const std::vector<User>& users)
{
    ...
    if (out.is_open())
    {
        ...
        for (const auto& user : users)
        {
            ...
            {
                out << withNull(privi.database_name) << " "
                    << withNull(privi.table_name) << " " << privi.number << " ";
            }
            out << std::endl;
        }
        out.close();
                                ...
                                std::vector<Table>& tables)
{
    ...
    // each table
    // std::cout << databaseName << ": " << tableNames.size() << " Tables" <<
    // std::endl;
            ...
            else if (type == kDefault)
            {
                ...
                constraints.push_back(
                    new DefaultConstraint(fieldName, constraintName,
                                          ColasqlTool::GetAnyByTypeAndValue(
                                              field_map.at(fieldName), value)));
            }
            else
            {
